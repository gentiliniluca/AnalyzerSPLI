 ...
 * sia uguale ad 123 vuol dire che ci troviamo nel caso del protocollo ntp
        		...
        		if ((* (char *)p) == '\n') {
		  		...
		  		if (first_line) {
					...
					buf2 = strtok((char *)buffer, " ");
            				if (flag_ftp_req && buf2!=NULL && control_header_req(buf2)) { /*TODO check control_header_req(...) in util.c*/
            					printf("Status-Line: ");
              					printf("%s\n", aux);
						/*
						printf("%s ", buf2);
						...
						*/
            					...
            					if (flag_ftp_res && buf2!=NULL && control_header_res(buf2)) { /*TODO check control_header_res(...) in util.c*/
		            				printf("Status-Line: ");
		            				printf("%s\n", aux);
							//printf("%s ", buf2);
							//buf2 = strtok(NULL, " ");
